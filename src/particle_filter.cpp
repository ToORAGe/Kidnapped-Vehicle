...
void ParticleFilter::init(double x, double y, double theta, double std[]) {
  ...
  normal_distribution<double> dist_the(theta,std[2]);
    num_particles = 200;
...
void ParticleFilter::prediction(double delta_t, double std_pos[], double velocity, double yaw_rate) {
  ...
  for(int i =0; i<num_particles; i++)
  {
    ...
    delta_x = delta_x + dist_x(gen);
    delta_y = delta_y + dist_y(gen);
    delta_theta = delta_theta + dist_theta(gen);

    particles[i].x = x + delta_x;
    particles[i].y = y + delta_y;
    particles[i].theta = theta + delta_theta;
    }
  ...
  for (unsigned int i =0; i<observations.size(); i++)
  {
    ...
    for(unsigned int j =0; j<predicted.size(); j++)
    {
     cur_dist=  dist(predicted[j].x,predicted[j].y,observations[i].x,observations[i].y);
      ...
      //index_min = std::min_element(distances.begin(),distances.end())-distances.begin();
      //observations[i].id = predicted[index_min].id;
      //distances.clear();
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
  ...
  for (int i = 0; i < num_particles; i++) {
    ...
    for (unsigned int j = 0; j < transformed_os.size(); j++) {
      ...
      double pred_weight = ( 1/(2*M_PI*s_x*s_y)) * exp( -( pow(pr_x-o_x,2)/(2*pow(s_x, 2)) + (pow(pr_y-o_y,2)/(2*pow(s_y, 2))) ) );

      // product of this obersvation weight with total observations weight
      particles[i].weight *= pred_weight;
    }
  }
}
